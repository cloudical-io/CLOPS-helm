...
def merge_dicts(a, b, path=None, diff=None):
    """Merges b into a, track deletions, and returns dictionary with additions, changes and deletions"""

    if path is None: path = []
    if diff is None: diff = {'add_or_change': {}, 'delete': {}}

    # Check for additions and changes
    for key in b:
        if key in a:
            if isinstance(a[key], dict) and isinstance(b[key], dict):
                if key not in diff['add_or_change']:
                    diff['add_or_change'][key] = {}
                merge_dicts(a[key], b[key], path + [str(key)], {'add_or_change': diff['add_or_change'][key], 'delete': diff['delete']})
            elif a[key] == b[key]:
                pass  # same leaf value
            else:
                #print('Changed in {}: {} -> {}'.format('.'.join(path + [str(key)]), a[key], b[key]))
                diff['add_or_change'][key] = copy.deepcopy(b[key])  # record changes
        else:
            #print('Added {}: {}'.format('.'.join(path + [str(key)]), b[key]))
            diff['add_or_change'][key] = copy.deepcopy(b[key])  # record additions

    # Check for deletions
    for key in a:
        if key not in b:
            #print('Deleted {}: {}'.format('.'.join(path + [str(key)]), a[key]))
            diff['delete'][key] = copy.deepcopy(a[key])  # record deletions
...
def main():
    ...
    #print('Changes, additions, and deletions from {} to {}:\n'.format(args.yaml1, args.yaml2))
